...
function ClaimButton({ item }) {
  ...
  const buttonLabel = itemType === 'found' ? 'Claim item' : 'Report found'
...
function openClaimModal(item) {
  ...
  const fullname = currentUser?.username || currentUser?.email || ''
  ...
  <div style="position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(2,6,23,0.6);z-index:9999">
    <div style="background:#031025;padding:20px;border-radius:12px;max-width:480px;width:90%;color:#e6eef8">
      ...
      <p style="margin:0 0 12px">Please confirm your name and email to submit the claim.</p>
      <input id="clf-claim-name" placeholder="Full name" value="${fullname}" style="width:100%;padding:10px;margin-bottom:8px;border-radius:8px;border:1px solid #1f2937;background:#0b1220;color:#e6eef8" />
      <input id="clf-claim-email" placeholder="Email" value="${email}" style="width:100%;padding:10px;margin-bottom:12px;border-radius:8px;border:1px solid #1f2937;background:#0b1220;color:#e6eef8" />
  ...
  document.getElementById('clf-claim-cancel').onclick = () => { document.getElementById('clf-claim-modal')?.remove() }
    ...
    try {
      const user = currentUser || JSON.parse(localStorage.getItem('clf_user') || 'null')
      const payload = {
        user_id: user?.user_id || null,
        item_id: item.id,
        proof: `Claim by ${name} <${email}>`,
        claimant_name: name,
